...
def _base_provide(path, **kwargs):
    def decorator(func):
        ...
        getattr(func, conf.decorate).update(kwargs)
        getattr(func, conf.decorate).update({
            "path": path,
            "name": name
        })
        return func
...
def discover(cls):
    return [
        (m[0], m[1], getattr(m[1], conf.decorate))
        for m in inspect.getmembers(cls) if hasattr(m[1], conf.decorate)
    ]


class Application(object):
    """
    The basic application. It provides routing and request dispatching
    """
    ...
    def dispatch(
            self, method, path, query=None, body=None, headers=None,
...
class WSGIAppMixin(object):
    ...
    def prepare(self, func, kwargs, context):
        opts = getattr(func, conf.decorate)
